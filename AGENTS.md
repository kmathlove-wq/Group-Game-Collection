...
- `/geometry-dash`는 서버 방 없이 Canvas로 직접 그린 1인용 점프 액션 게임이다. 원작 그래픽·음원은
  ...
  전체 통합 순위이며, `GET/POST /api/geometry-dash/scores`로 조회·제출한다(`SCORE_MAX`, IP당 2초
  쿨다운으로만 검사, 물리 재현 없음). 가시·블록·구덩이 외에 누르면 뜨고 떼면 떨어지는 비행 구간이
  있다. 점프·비행 물리 상수를 바꾸면 통과 가능성을 오프라인 시뮬레이션으로 먼저 검증한다.
