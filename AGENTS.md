...
- `game:secret`은 방장과 출제자의 개인 socket ID에만 보낸다.
...
- 제시어는 직접 입력, 준비 목록 선택, 난이도별 무작위 선택을 지원한다.
...
- 모바일 기본 탭은 그림이며 그림/채팅/참가자/그림 도구 순서다.
- 태블릿은 참가자/그림판 왼쪽 열과 상시 표시되는 채팅 오른쪽 열을 사용하고, 휴대폰만 모바일 탭 레이아웃으로 전환한다.
