...
현재 그림 퀴즈 `두들팡`, 노래 퀴즈 `송캐치`, 1인용 `절대 못 맞히는 퀴즈쇼`를 제공한다.
...
## 게임 컬렉션 구조
- 루트 `/`는 전체 게임 선택 화면이며 실제 제공 중인 세 게임을 같은 수준의 카드로 표시한다.
...
- 퀴즈쇼 저장 점수는 답안 `history`를 다시 채점해 복구하며, 20번 진입·제출 시에도 저장된 `score` 숫자 대신 앞선 답안을 즉시 재채점한다.
