...
현재 그림 퀴즈 `두들팡`과 노래 퀴즈 `송캐치`를 제공한다.
...
- 화면상 이름은 `그룹 게임 컬렉션-두들팡`, `그룹 게임 컬렉션-송캐치`이며 송캐치 단체전 PC 화면도 참가자/문제판/채팅 3열을 사용한다.
...
## 게임 컬렉션 구조
- 루트 `/`는 전체 게임 선택 화면이며 `두들팡`과 `송캐치`를 같은 수준의 카드로 표시한다.
